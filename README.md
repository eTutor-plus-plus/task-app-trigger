# eTutor Task-App: Oracle Trigger

This application provides a REST-interface for following task type: trigger.
...
docker run -p 8090:8081 \ 
  -e SPRING_DATASOURCE_URL="jdbc:oracle:thin:@//localhost:1521/FREEPDB1" \
  -e SPRING_DATASOURCE_USERNAME=etutor \
  -e SPRING_DATASOURCE_PASSWORD=myPwd \
  -e SPRING_FLYWAY_USER=etutor \
  -e SPRING_FLYWAY_PASSWORD=myPwd \
  ...
  etutorplusplus/task-app-trigger
```
...
services:
    task-app-trigger:
        image: etutorplusplus/task-app-trigger
        ...
        environment:
            ...
            SPRING_FLYWAY_USER: etutor
            SPRING_FLYWAY_PASSWORD: myPwd
